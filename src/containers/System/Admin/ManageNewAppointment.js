...
class ManageNewAppointment extends Component {
    ...
    handleConfirmAppointment = async (item) => {
        ...
        let res = await confirmAppointment({
            id: item.id,
            email: item.Patient_Record.email,
            fullName: language === LANGUAGES.VI ? `${item.Patient_Record.lastName} ${item.Patient_Record.firstName}` : `${item.Patient_Record.firstName} ${item.Patient_Record.lastName}`,
            doctor: language === LANGUAGES.VI ? `${item.Schedule.doctorData.Doctor_Infor.lastName} ${item.Schedule.doctorData.Doctor_Infor.firstName}` : `${item.Schedule.doctorData.Doctor_Infor.firstName} ${item.Schedule.doctorData.Doctor_Infor.lastName}`,
            time: language === LANGUAGES.VI ? `${item.Schedule.timeTypeData.valueVi} - ${moment(new Date(Number(item.Schedule.date))).format('DD/MM/YYYY')}` : `${item.Schedule.timeTypeData.valueEn} - ${moment(new Date(Number(item.Schedule.date))).format('DD/MM/YYYY')}`,
            clinic: item.Schedule.doctorData.Doctor_Infor.Clinic.name,
            language: language
                ...
                >
                    <div className='manage-approval-container container'>
                        <div className='title text-center'>
                            {language === LANGUAGES.VI ? 'Lịch hẹn mới' : 'New appointment'}
                        </div>
                        <input
                        ...
                        <table className='table'>
                            ...
                            <tbody>
                                {listBooking.length > 0 ?
                                        ...
                                        {listBooking.map((item, index) => {
                                            return (
                                                <tr key={`bk-${index}`}>
                                                    ...
                                                    <td>
                                                        {item.statusId === 'S2' && <button onClick={() => this.handleConfirmAppointment(item)}>{language === LANGUAGES.VI ? 'Duyệt' : 'Approve'}</button>}
                                                        <button onClick={() => this.handleOpenCancelAppointment(item)}>{language === LANGUAGES.VI ? 'Huỷ' : 'Cancel'}</button>
                                                    </td>
                                                </tr>
                                            )
                                        })}
                                    ...
                                    : <tr>
                                        <td colSpan={7} className='text-center'>{language === LANGUAGES.VI ? 'Không có dữ liệu' : 'No data'}</td>
                                    </tr>
                                }
                            </tbody>
                        </table>
